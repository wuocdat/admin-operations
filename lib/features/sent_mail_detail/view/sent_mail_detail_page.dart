...
import 'package:tctt_mobile/features/sent_mail_detail/cubit/sent_mail_detail_cubit.dart';
...
class SentMailDetailPage extends StatelessWidget {
  const SentMailDetailPage({super.key, required this.mailId});

  static Route<bool> route(String mailId) {
    return MaterialPageRoute<bool>(
      builder: (_) => BlocProvider(
        create: (context) => SentMailDetailCubit(
            mailRepository: context.read<MailRepository>(), mailId: mailId)
          ..fetchMail(),
        child: SentMailDetailPage(mailId: mailId),
      ),
    );
  }
        ...
        child: Container(
          ...
          child: SingleChildScrollView(
            child: Padding(
              ...
              child: BlocBuilder<SentMailDetailCubit, SentMailDetailState>(
                builder: (context, state) {
                  return Column(
                    mainAxisSize: MainAxisSize.max,
                    children: [
                      ContentContainer(
                        sender: state.mail.receivers
                            .map((item) => item['name'])
                            .join(', '),
